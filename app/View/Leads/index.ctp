<div class="span2"><h2><?php echo __('Leads'); ?></h2></div>
<div class="span6" style="padding: 20px;">
	<i class="glyphicon glyphicon-hand-right"></i>&nbsp;&nbsp;
	<?php
	$odesk_w = '';
	$elance_w = '';
	if (!empty($this->params["pass"]) && in_array("odesk", $this->params["pass"])) {
		$odesk_w = "text-decoration:underline";
	} elseif (!empty($this->params["pass"]) && in_array("elance", $this->params["pass"])) {
		$elance_w = "text-decoration:underline";
	}

	if (!empty($this->params["pass"][0]) && $this->params["pass"][0] != "odesk" && $this->params["pass"][0] != "elance") {

		$id = $this->params["pass"][0];
	} else {
		$id = null;
	}
	echo $this->Html->link('<span style="margin-right:10px;">oDesk</span>', array("controller" => "leads", "action" => "index", $id, "odesk"), array("escape" => false, "style" => $odesk_w));

	echo $this->Html->link("Elance", array("controller" => "leads", "action" => "index", $id, "elance"), array("style" => $elance_w));
	?>

</div>

<div class="span2 text-right" style="padding: 20px;"><?php echo $this->Html->link('<i class="glyphicon glyphicon-plus"></i>Add', array("controller" => "leads", "action" => "add"), array("title" => "Add private lead", "escape" => false)); ?></div>
			...
			if (!empty($leads)) {
				foreach ($leads as $lead):
					...
					<tr <?php if (!empty($Awdprojects) && array_key_exists($lead['Lead']['id'], $Awdprojects)) { ?>style="background-color:#EDF7ED;"<?php } ?> >
						...
						<td><?php
							...
							?>&nbsp;
							<br>
				<date class=" text-right" style="font-size:12px;"><?php echo $lead['Lead']['posted_on']; ?></date>
				</td>
				...
				<td width="10%" class="statusList"><?php
					if (!empty($Awdprojects) && array_key_exists($lead['Lead']['id'], $Awdprojects)) {
						...
						$staus = array(1 => "Open bid", 2 => "Bid Placed", 3 => "Declined", 4 => "Feedback", 5 => "Project start");
						...
						<div class="statusListdiv">
							<div id="mainTxt<?php echo $lead['Lead']['id']; ?>">
								...
								for ($i = 1; $i <= $mystatus; $i++) {
									...
									echo "<span style='$txtdecostyle'>" . $staus[$i] . "</span><br/>";
								}
								if ($mystatus == 3) {
									echo "<span style='text-decoration:none'>" . $staus[$mystatus] . "</span><br/>";
								}
								?>
							</div>
